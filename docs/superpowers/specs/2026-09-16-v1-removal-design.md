...
## 7. Найдено по пути (финальное ревью)

Мостик незаметно давал одно свойство, которого теперь нет: `LegacyIdentity.parse` читал и
v2-раскладку, поэтому адаптер на старом v2-релизе, чей `/status` не содержал поля, которое
приложение с тех пор сделало обязательным, всё равно доходил до `.updating`. Теперь такое тело —
`.faulty` и в лестнице запуска, и в наблюдении за перезагрузкой, а выход — только кабель. Это
ровно выбор §2/§6, но из него следует дисциплина контракта, которую стоит помнить: **новое поле
в `/status` адаптера — nullable/optional, пока его не отдаёт каждая плата в поле; иначе оно
идёт вместе с поднятием `proto`.** То же для машинки, хотя её версия приходит с hello и до
`/status` дело доходит реже.
