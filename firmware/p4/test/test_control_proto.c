...
#include <stdio.h>
#include <string.h>
/* The datagram cap is the schema's, not this test's — a frame that is legal here and
   rejected on the car (or the reverse) is exactly the class of bug the generator exists
   to remove. */
...
static control_frame_t parse(const char *msg) {
    control_frame_t f;
    int r = control_parse_frame(msg, strlen(msg), RT_MAX_DATAGRAM, &f);
    if (r != 0) {
...
static void bad(const char *msg) {
    ...
    int r = control_parse_frame(msg, len, RT_MAX_DATAGRAM, &f);
    if (r != -1) {
...
int main(void) {
    ...
    char big[RT_MAX_DATAGRAM * 2];
    const char *head = "{\"seq\":1,\"t\":0.5,\"y\":0.5,\"pad\":\"";
    int n = (int)strlen(head);
    memcpy(big, head, (size_t)n);
    while (n < RT_MAX_DATAGRAM - 2) big[n++] = 'x';
    big[n++] = '"';
    big[n++] = '}';
    assert(n == RT_MAX_DATAGRAM);
    control_frame_t o;
    assert(control_parse_frame(big, RT_MAX_DATAGRAM, RT_MAX_DATAGRAM, &o) == 0);
    big[n - 1] = 'x';                 /* one byte over, still valid JSON if it were read */
    big[n++] = '"';
    big[n++] = '}';
    assert(control_parse_frame(big, (size_t)n, RT_MAX_DATAGRAM, &o) == -1);
    ...
    assert(control_parse_frame(raw, 16, RT_MAX_DATAGRAM, &part) == 0);
