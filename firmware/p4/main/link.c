...
#include "board.h"     /* BOARD_KICK_DUTY, BOARD_KICK_TICKS */
#include "pca9685.h"
#include "ramp.h"
...
static void link_task(void *arg) {
    ...
    for (;;) {
        ...
        /* Start-assist state, one countdown per channel. Lives with the task because
           only this loop, the sole writer, knows when a channel truly left standstill. */
        static uint8_t s_kick[8] = {0};
        uint16_t up[8];
        link_kick_plan(s_current, tgt, up, s_kick,
                       ramp_max_up_per_tick(ramp_get_ms(), LINK_TICK_MS),
                       BOARD_KICK_DUTY, BOARD_KICK_TICKS);
        uint16_t next[8];
        uint8_t  order[8];
