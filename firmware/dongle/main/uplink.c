...
{
    return atomic_load(&u->failures) >= UPLINK_DEAD_AFTER;
}
...
bool uplink_failed(uplink_t *u, uint32_t now_ms)
{
    ...
    uint32_t n = atomic_load(&u->failures);
    while (n < UINT32_MAX && !atomic_compare_exchange_weak(&u->failures, &n, n + 1u)) {}
