...
#
#   firmware/car/modem/flash-radio.sh                 # build only, then print the flash command
#   firmware/car/modem/flash-radio.sh /dev/cu.usbXXX  # build and flash through the C6 UART header
set -euo pipefail
ROOT="$(cd "$(dirname "$0")/../../.." && pwd)"
HOSTED="$ROOT/firmware/car/core/managed_components/espressif__esp_hosted"
...
PIN=$(grep -E 'espressif/esp_hosted:' "$ROOT/firmware/car/core/main/idf_component.yml" | grep -oE '[0-9]+\.[0-9]+\.[0-9]+' | head -1) || true
[ -n "$PIN" ] || { echo "ERROR: no esp_hosted pin found in $ROOT/firmware/car/core/main/idf_component.yml"; exit 1; }
...
if [ $# -ge 1 ]; then
    ...
    echo "  firmware/car/modem/flash-radio.sh /dev/cu.usbserial-XXXX"
fi
